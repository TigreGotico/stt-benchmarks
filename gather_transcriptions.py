...
STTS = [
    ...
    # ("ovos-stt-plugin-fasterwhisper", "tiny.en", FasterWhisperSTT({"model": "tiny.en",
    #("ovos-stt-plugin-fasterwhisper", "small", FasterWhisperSTT({"model": "small",
    #                                                                "beam_size": 5,
    #                                                                "cpu_threads": 4
    #                                                                })),
    #("ovos-stt-plugin-fasterwhisper", "small.en", FasterWhisperSTT({"model": "small.en",
    #                                                                "beam_size": 5,
    #                                                                "cpu_threads": 4
    #                                                                }))
...
DATASETS = [(ds[0],
            ...
            for ds in [
                ("openslr/librispeech_asr", "clean", "test", True),
                # ("openslr/openslr", "SLR83", "train", True), # uk and ireland
                # ("openslr/openslr", "SLR70", "train", True), # english from nigeria
                # ("MushanW/GLOBE", "default", "test", True),
                ("mythicinfinity/Libriheavy-HQ", "small", "train", True),
                ("MLCommons/peoples_speech", "default", "test", True),
                ("mozilla-foundation/common_voice_17_0", "en", "test", True),
                ("google/fleurs", "en_us", "test", True),
                ("speechcolab/gigaspeech", "xs", "test", True),
                ("janaab/supreme-court-speech", "default", "test", True),
                ("facebook/voxpopuli", "en", "test", True),
                ("facebook/voxpopuli", "en_accented", "test", True),
            ]]
...
for d, dataset in DATASETS:
    ...
    if out_path in seen:
        out_path = f"{DL_PATH}/{d}/{dataset[1]}"
